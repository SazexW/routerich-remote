...
case "$HOSTNAME_SAFE" in
  *" "*|*"_"*|*"."*)
    fail "hostname не должен содержать пробелы, подчёркивания '_' или точки '.'. Используй дефис: router-client-01"
    ;;
esac

echo
echo "Роль роутера:"
echo "  1 = admin/main роутер: разрешить LAN -> Tailscale, чтобы устройства локальной LAN могли открывать удалённые 100.x.x.x"
